...
def _refresh_top_level_constants() -> None:
    ...
    from config import browser, openai_protocol, proxy as _proxy, register, email, twofa, roxybrowser, cloakbrowser, camoufox, browser_use, skyvern, codex, extract_link, sub2api, chatgpt2api, humanize, flow_trigger
    # 简单粗暴：枚举一遍重要常量，覆盖到 _self
    for src in (browser, openai_protocol, _proxy, register, email, twofa, roxybrowser, cloakbrowser, camoufox, browser_use, skyvern, codex, extract_link, sub2api, chatgpt2api, humanize, flow_trigger):
        for k in dir(src):
...
__all__ = [
    ...
    # proxy
    "PROXY_POOL", "PROXY_RESIN_MODE", "PROXY_RESIN_TEMPLATE",
